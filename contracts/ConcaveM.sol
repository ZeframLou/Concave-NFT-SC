...
contract ConcaveNFT is ERC721Enumerable, Pausable, Ownable {
    ...
    mapping(uint256 => uint256) public tokenHasMinted;
        ...
        returns (uint256)
    {
        ...
        if (totalSupply() >= colors_quota) {
            ...
            // Mint left can be required here, problem is with batch minting, maube mintAmount can be passed to mintLeft.
    ...
    function mintLeft (address user) public returns (bool){
        uint256 [] memory ownedTokens = new uint[](4317);
        ownedTokens = getColorsOwnedByUser(user);
        bool canMint = false;
        for (uint i = 0; i < ownedTokens.length ; i++){ 
            if (tokenHasMinted[ownedTokens[i]]< 2){
                canMint = true;
                tokenHasMinted[ownedTokens[i]]++;
                return canMint;
            }
        }
        return canMint;
